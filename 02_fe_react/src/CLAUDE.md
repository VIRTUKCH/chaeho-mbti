...
**책임:** React 앱 소스. **4카드 모자이크 구조** (연애 / 가족 / 친구 / 직장(학교)) — 검사 종료 후 카드 4장이 노출되고, 사용자가 한 장씩 펴서 결과를 본다. README가 단일 소스 진실 원천.
...
- [App.tsx](App.tsx) — 라우터 라이브러리 없이 쿼리 파라미터로 분기.
  ...
  - `?view=mosaic` → [pages/Mosaic.tsx](pages/Mosaic.tsx) (4카드 모자이크)
  - `?card=love|family|friend|work[&type=…]` → [pages/CardDetail.tsx](pages/CardDetail.tsx)
- [cards/](cards/) — 카드 데이터(연애·가족·친구·직장)와 메타 + 결과 조회 헬퍼.
...
- **전체 사용자 카피는 학술 어휘를 쓰지 않는다.** 백엔드 매핑(Sternberg / Adler / K-EQ / 미정)은 [cards/love.ts](cards/love.ts) 등 파일 주석에만 적고, 화면에는 안 노출.
- **카드 우선순위 = 연애 > 가족 > 친구 > 직장(학교).** [cards/meta.ts](cards/meta.ts)의 `CARD_ORDER` 가 그 순서. 모자이크 노출·카피 다듬기 순서가 모두 여기서 결정. 직장 카드는 현재 학술 매핑 미정 + 카피 초안 상태.
...
1. [cards/](cards/) 의 해당 카드 파일(`love.ts` / `family.ts` / `friend.ts` / `work.ts`)에 `CardResult` 객체 추가.
