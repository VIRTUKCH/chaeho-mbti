// 4카드 모자이크: 연애 / 가족 / 친구 / 직장(학교)
// 각 카드는 백엔드 학술 척도(Sternberg/Adler/K-EQ/미정)로 정답을 산출하지만
// 사용자 노출 카피는 학술 어휘를 쓰지 않는다 (학술 백엔드 / 밈 프론트엔드 분리).

export type CardId = 'love' | 'family' | 'friend' | 'work'

export type CardMeta = {
  id: CardId
  emoji: string
  label: string // 연애 / 가족 / 친구 / 직장(학교)
...
export type CardResult = {
  cardId: CardId
  typeId: string // 'consummate' / 'oldest' / 'high' / 'moodMaker' 등
  badge: string // 짧은 라벨 ("완성된 사랑형", "맏이형", "공감 만렙형", "분위기 메이커형")
